...
export function AdvancePaymentModal({ isOpen, onClose, onSuccess }: AdvancePaymentModalProps) {
  ...
  const createAdvancePaymentMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data?.data?.account_payable_id) {
        queryClient.invalidateQueries({ queryKey: [`/api/account-payables/${data.data.account_payable_id}`] });
        queryClient.invalidateQueries({ queryKey: ['/api/transactions/account-payable', data.data.account_payable_id] });
      }
  ...
  // Get selected vendor and account for confirmation modal
  ...
  const selectedAccount = useMemo(
    () => accounts.find((acc: any) => acc.id === pendingTransaction?.source_account_id),
    [accounts, pendingTransaction?.source_account_id]
  );
  ...
  return (
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">Advance Payment</DialogTitle>
        </DialogHeader>
    ...
    <Dialog open={isConfirmOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="w-[95vw] max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">Confirm Advance Payment</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Are you sure you want to create this advance payment?
          </p>
