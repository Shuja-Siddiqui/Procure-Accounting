...
export function AccountReceivableLedgerFilters({
  onApply,
  isLoadingLedger,
}: FiltersProps) {
  ...
  const { data: accountsResponse, isLoading: isAccountsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        throw new Error("Failed to fetch account receivables");
      }
      return res.json();
    },
  });

  const accounts: AccountReceivable[] = accountsResponse?.data ?? [];
