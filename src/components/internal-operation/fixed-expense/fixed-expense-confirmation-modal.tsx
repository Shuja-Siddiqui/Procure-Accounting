...
interface FixedExpenseConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  formData: any;
}

export function FixedExpenseConfirmationModal({ isOpen, onClose, formData }: FixedExpenseConfirmationModalProps) {
  ...
  const createFixedExpenseMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Error",
        description: error?.response?.data?.message || error?.message || "Failed to create fixed expense",
        variant: "destructive",
      });
    },
  });
  ...
  const handleConfirm = () => {
    createFixedExpenseMutation.mutate(formData);
  };
  ...
  return (
    <ConfirmationModal
      ...
      isLoading={createFixedExpenseMutation.isPending}
