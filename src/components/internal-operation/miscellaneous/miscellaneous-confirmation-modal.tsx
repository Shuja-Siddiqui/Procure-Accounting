...
interface MiscellaneousConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  formData: any;
}

export function MiscellaneousConfirmationModal({ isOpen, onClose, formData }: MiscellaneousConfirmationModalProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const { user } = useAuth();

  const createMiscellaneousMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Miscellaneous transaction created successfully",
      });
    ...
    onError: (error: any) => {
      console.error('Error creating miscellaneous transaction:', error);
      toast({
        title: "Error",
        description: error?.response?.data?.message || error?.message || "Failed to create miscellaneous transaction",
        variant: "destructive",
      });
    },
  });
  ...
  const handleConfirm = () => {
    createMiscellaneousMutation.mutate(formData);
  };
  ...
  return (
    <ConfirmationModal
      ...
      isLoading={createMiscellaneousMutation.isPending}
